<svelte:options customElement="qsr-button-group" />
...
<div class="button-group button-group--{orientation}" role="group">
  {#each parsedItems as item}
    <button class="button button--{item.variant || 'secondary'}"
      on:click={() => dispatchEvent(new CustomEvent('qsr:button-group:click', { detail: item, bubbles: true, composed: true }))}
    >{item.label}</button>
  {/each}
</div>
