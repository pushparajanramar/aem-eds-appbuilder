<svelte:options customElement="qsr-toggle-switch" />
...
<script>
  ...
  function handleChange(e) { dispatchEvent(new CustomEvent('qsr:toggle-switch:change', { detail: { name, value: e.target.value, checked: e.target.checked }, bubbles: true, composed: true })); }
</script>

<label class="toggle-switch {disabled === 'true' || disabled === true ? 'toggle-switch--disabled' : ''}">
  {#if labelleft}<span class="toggle-switch__label-text">{labelleft}</span>{/if}
  <input class="toggle-switch__input" type="checkbox" role="switch" name={name}
  ...
  <span class="toggle-switch__label-text">{labelright || label}</span>
</label>
