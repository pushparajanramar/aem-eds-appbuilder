<svelte:options customElement="qsr-timeline" />
...
<ol class="timeline timeline--{orientation}">
  {#each parsedItems as item, i}
    <li class="timeline__item">
      <div class="timeline__marker" aria-hidden="true">{item.icon || (i + 1)}</div>
      <div class="timeline__content">
        {#if item.date}<time class="timeline__date" datetime={item.date}>{item.date}</time>{/if}
        {#if item.heading}<h3 class="timeline__heading">{item.heading}</h3>{/if}
        {#if item.body}<p class="timeline__body">{@html item.body}</p>{/if}
      </div>
    </li>
  {/each}
</ol>
