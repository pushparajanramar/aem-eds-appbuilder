<svelte:options customElement="qsr-testimonials" />
  ...
  {#each parsedItems as item}
    <blockquote class="testimonials__item">
      ...
      <footer class="testimonials__footer">
        {#if item.avatar}<img class="testimonials__avatar" src={item.avatar} alt={item.author} loading="lazy" />{/if}
        <cite class="testimonials__author">{item.author}</cite>
        {#if item.role}<span class="testimonials__role">{item.role}</span>{/if}
      </footer>
    </blockquote>
  {/each}
</section>
