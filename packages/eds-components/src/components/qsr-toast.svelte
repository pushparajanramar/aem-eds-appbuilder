<svelte:options customElement="qsr-toast" />
...
<script>
  ...
  $: isVisible = visible === 'true' || visible === true;
  
  function dismiss() {
    isVisible = false;
    dispatchEvent(new CustomEvent('qsr:toast:dismiss', { bubbles: true, composed: true }));
  }
  $: { if (visible === 'true' || visible === true) { setTimeout(dismiss, +duration); } }
</script>
...
<div class="toast-container toast-container--{position}">
  {#if isVisible}
    <div class="toast toast--{type}" role="status" aria-live="polite" aria-atomic="true">
      ...
      <button class="toast__close" aria-label="Dismiss notification" on:click={dismiss}>&#10005;</button>
    </div>
  {/if}
</div>
