<svelte:options customElement="qsr-progress-bar" />
...
<div class="progress-bar progress-bar--{variant}" role="progressbar"
  aria-valuenow={+value} aria-valuemin="0" aria-valuemax={+max} aria-label={label}>
  <div class="progress-bar__track">
    <div class="progress-bar__fill" style="width: {Math.min(100, (+value / +max) * 100)}%"></div>
  </div>
  {#if showlabel === 'true' || showlabel === true}
    <span class="progress-bar__label">{label || `${Math.round((+value / +max) * 100)}%`}</span>
  {/if}
</div>
