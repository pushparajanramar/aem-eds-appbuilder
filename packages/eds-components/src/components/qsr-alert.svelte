<svelte:options customElement="qsr-alert" />
...
<script>
  ...
  function dismiss() {
    dispatchEvent(new CustomEvent('qsr:alert:dismiss', { bubbles: true, composed: true }));
  }
</script>

<div class="alert alert--{type}" role="alert" aria-live="polite">
  <span class="alert__message">{@html message}</span>
  {#if dismissible}
    <button class="alert__close" aria-label="Dismiss" on:click={dismiss}>&#10005;</button>
  {/if}
</div>
