<svelte:options customElement="qsr-popover" />
...
<div class="popover-wrapper">
  <button class="popover__trigger" aria-haspopup="true" aria-expanded={isOpen}
    on:click={toggle}>{trigger}</button>
  {#if isOpen}
    <div class="popover popover--{placement}" role="tooltip">
      <div class="popover__content">{@html content}</div>
      <button class="popover__close" aria-label="Close" on:click={() => (isOpen = false)}>&#10005;</button>
    </div>
  {/if}
</div>
