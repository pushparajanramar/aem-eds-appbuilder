<svelte:options customElement="qsr-banner" />
...
<section class="banner">
  {#if imageurl}
    <img class="banner__img" src={imageurl} alt={heading} loading="lazy" />
  {/if}
  <div class="banner__content">
    {#if heading}<h2 class="banner__heading">{heading}</h2>{/if}
    {#if body}<p class="banner__body">{@html body}</p>{/if}
    {#if ctaHref}
      <a class="banner__cta" href={ctaHref}>{ctaLabel || 'Learn more'}</a>
    {/if}
  </div>
</section>
