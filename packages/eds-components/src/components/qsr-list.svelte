<svelte:options customElement="qsr-list" />
...
{#if ordered === 'true' || ordered === true}
  <ol class="list list--{variant}">{#each parsedItems as item}<li class="list__item">{@html item}</li>{/each}</ol>
{:else}
  <ul class="list list--{variant}">{#each parsedItems as item}<li class="list__item">{@html item}</li>{/each}</ul>
{/if}
