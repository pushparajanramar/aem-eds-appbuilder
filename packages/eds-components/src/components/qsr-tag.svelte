<svelte:options customElement="qsr-tag" />
...
<span class="tag tag--{variant}">
  {#if icon}<span class="tag__icon" aria-hidden="true">{icon}</span>{/if}
  <span class="tag__label">{label}</span>
  {#if removable === 'true' || removable === true}
    <button class="tag__remove" aria-label="Remove {label}"
      on:click={() => dispatchEvent(new CustomEvent('qsr:tag:remove', { detail: { label }, bubbles: true, composed: true }))}>&#10005;</button>
  {/if}
</span>
