<svelte:options customElement="qsr-slider" />
...
<script>
  ...
  function handleInput(e) { currentValue = e.target.value; dispatchEvent(new CustomEvent('qsr:slider:input', { detail: { name, value: currentValue }, bubbles: true, composed: true })); }
</script>

<div class="slider">
  {#if label}<label class="slider__label" for="sl-{name}">{label}</label>{/if}
  <div class="slider__track">
    <input class="slider__input" id="sl-{name}" type="range" name={name}
      min={min} max={max} step={step} bind:value={currentValue}
