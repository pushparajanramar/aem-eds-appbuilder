<svelte:options customElement="qsr-button" />
...
{:else}
  <button class="button button--{variant}" type={type} disabled={disabled === 'true' || disabled === true}
    on:click={() => dispatchEvent(new CustomEvent('qsr:button:click', { detail: { label }, bubbles: true, composed: true }))}
  >{label}</button>
{/if}
