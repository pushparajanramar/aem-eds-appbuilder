<svelte:options customElement="qsr-dropdown-menu" />
...
<script>
  ...
  function select(item) { isOpen = false; dispatchEvent(new CustomEvent('qsr:dropdown-menu:select', { detail: item, bubbles: true, composed: true })); }
</script>

<div class="dropdown-menu">
  <button class="dropdown-menu__trigger" aria-haspopup="true" aria-expanded={isOpen}
    on:click={toggle}>{label} <span aria-hidden="true">&#9660;</span></button>
  {#if isOpen}
    <ul class="dropdown-menu__list" role="menu">
      {#each parsedItems as item}
        <li role="menuitem">
          <a class="dropdown-menu__item" href={item.href || '#'}
            on:click={() => select(item)}>{item.label}</a>
        </li>
      {/each}
    </ul>
  {/if}
