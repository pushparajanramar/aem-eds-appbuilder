<svelte:options customElement="qsr-date-picker" />
...
<script>
  ...
  function handleChange(e) { dispatchEvent(new CustomEvent('qsr:date-picker:change', { detail: { name, value: e.target.value }, bubbles: true, composed: true })); }
</script>

<div class="date-picker">
  {#if label}<label class="date-picker__label" for="dp-{name}">{label}</label>{/if}
  <input class="date-picker__input" id="dp-{name}" type="date"
    name={name} value={value} min={min} max={max} placeholder={placeholder}
