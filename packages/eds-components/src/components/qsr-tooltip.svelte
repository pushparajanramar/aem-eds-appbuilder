<svelte:options customElement="qsr-tooltip" />
...
<span class="tooltip-wrapper">
  <span class="tooltip__trigger" tabindex="0" aria-describedby="tooltip-desc"
    on:mouseenter={() => (isVisible = true)}
    on:mouseleave={() => (isVisible = false)}
    on:focusin={() => (isVisible = true)}
    on:focusout={() => (isVisible = false)}>{trigger}</span>
  {#if isVisible}
    <div class="tooltip tooltip--{placement}" id="tooltip-desc" role="tooltip">{content}</div>
  {/if}
</span>
