<svelte:options customElement="qsr-rating-stars" />
...
<script>
  ...
  function rate(star) {
    if (readonly === 'true' || readonly === true) return;
    dispatchEvent(new CustomEvent('qsr:rating-stars:change', { detail: { value: star }, bubbles: true, composed: true }));
  }
</script>

<div class="rating-stars" role={readonly === 'true' || readonly === true ? 'img' : 'group'}
  aria-label={label || `Rating: ${value} out of ${max} stars`}>
  {#each Array.from({length: +max}, (_, i) => i + 1) as star}
    <button class="rating-stars__star {+value >= star ? 'rating-stars__star--filled' : ''}"
      aria-label="{star} star{star !== 1 ? 's' : ''}"
      disabled={readonly === 'true' || readonly === true}
      on:click={() => rate(star)}>&#9733;</button>
  {/each}
</div>
